...
fn generation(san: &str) -> Generation {
    ...
    ca_params
        .distinguished_name
        .push(DnType::CommonName, "yadgar-iam rotation test authority");
...
impl Mount {
    ...
    fn new(files: &Generation) -> Self {
        let root = std::env::temp_dir().join(format!("yadgar-iam-rotation-{}", unique()));
...
/// CA from `Inputs::enrolment` used to pass every case in this file.
...
/// How `iam-db` is verified, likewise.
...
async fn an_atomic_swap_of_a_new_certificate_ends_the_watch() {
    ...
    swap_shortly(&mount, generation("iam"));
...
/// between — the `iam-db` dial, the broker connect — would be a window in which
...
/// THE ENROLMENT CA IS IN THE WATCH SET, and nothing else in this file proves
/// it.
///
/// Every rotation case builds its inputs through the same three methods, so a
/// `rotate::Inputs::enrolment` that quietly did nothing would leave them all
/// green — measured, and it did. This asserts MEMBERSHIP, which is the only
...
/// enrolment CA still has something to watch — which is what a default install
/// of this chart actually is.
#[tokio::test]
async fn each_configured_half_contributes_on_its_own() {
    ...
    // A gateway with a publicly-trusted certificate reads no CA at all, so
    // there is no path to watch even though the config exists.
    let no_ca = EnrolmentConfig::load("https://gateway.invalid:18443", None)
        .expect("no CA is a deployment, not an error");
    assert!(Inputs::default().enrolment(Some(&no_ca)).is_empty());
}
