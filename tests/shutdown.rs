...
//! connection, so what is lost is everything that connection was carrying. In
//! this service that is logins, credential resolutions and enrolment
//! redemptions.
//!
//! **AND `rotate` DEPENDS ON IT.** The rotation watcher's whole value is that
//! its self-exit drains, and it drains by resolving beside this future. Left
//! unfixed, the watcher would have selected against a shutdown path Kubernetes
//! never triggers.
//!
...
//!
//! Mirrored from `task/tests/shutdown.rs`, which fixed this first. `iam-db` and
//! `gateway` still need the same copy.
...
async fn a_sigterm_drains_the_server_instead_of_killing_the_process() {
    ...
    // router that needs no crypto keys and no `iam-db` channel keeps the rig
    // honest about that.
