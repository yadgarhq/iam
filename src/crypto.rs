...
// The `cfg(test)` verification counter, and the `PasswordHasher` that feeds it.
// Its reasoning travels with it; see the module.
#[cfg(test)]
mod counting;
#[cfg(test)]
use counting::CountingArgon2;
// RE-EXPORTED RATHER THAN MOVED OUT OF REACH: `service::tests` reads the
// counter as `crate::crypto::argon2_verifications`, which is where it has
// always been.
#[cfg(test)]
pub(crate) use counting::argon2_verifications;
