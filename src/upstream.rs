...
//! module-specific wiring on top: `iam-db`'s env-configured host, port and
//! transport, named the way `service.rs` and `main.rs` expect. `Iam` builds its
//! own typed `IamDbServiceClient` from the `Channel` this returns, the same way
//! `task`'s `Task` builds `TaskDbServiceClient` from the channel it is given —
//! the twin is cheap to clone and reconnects nowhere, so there is no reason to
//! hand around a pre-typed client instead of the channel itself.
//!
//! # TLS
//!
...
//! personal-data field (D72) crosses this connection, so a flat pod network
//! means anyone on it reads them.
//!
...
/// `<PREFIX>_TLS_DOMAIN`. This service has one upstream, `IAM_DB`; the gateway
...
/// `yadgar_dial` — one long-lived HTTP/2 connection per pod, re-resolved every
/// 5s — rather than a plain single-endpoint `Endpoint::connect`.
///
...
mod tests {
    ...
    fn nothing_configured_means_no_tls() {
        assert_eq!(UpstreamTls::from_lookup(IAM_DB, lookup(&[])).unwrap(), None);
    }
    ...
    fn a_ca_bundle_alone_does_not_enable_tls() {
        let vars = [("IAM_DB_TLS_CA_FILE", SENTINEL_CA)];
        assert_eq!(
